...
impl Plugin for Cie {
    ...
    fn activate(
        ...
        self.buffer = vec![vec![0.0; sample_rate * 5]; 2]; // Again 5 second max, but 2d array makes expanding from mono -> stereo and beyond easier.
    ...
    fn process(
        ...
        let mut pointer = pointer as usize;

        for mut channel_samples in buffer.iter_samples() {
            for (chan, sample) in channel_samples.iter_mut().enumerate() {
                self.buffer[chan][self.buffer_pointer] = *sample;
                *sample += self.buffer[chan][pointer] * feedback;
            }
            pointer += 1;
